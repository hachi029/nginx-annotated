...
// 在ngx_preinit_modules里统计得到静态模块的总数, 之后加上128，是模块数量的上限
ngx_uint_t         ngx_max_module;
// 模块计数器，模块数组的最后一个可用序号
static ngx_uint_t  ngx_modules_n;


// main()里调用,计算所有的静态模块数量，对每个module进行编号 index
...
ngx_preinit_modules(void)
{
    ...
    //赋值index和name。ngx_modules数却是在自动编译的时候生成的，位于objs/ngx_modules.c文件中
...
// cycle->pool内存池创建一个数组，可以容纳所有的模块，大小是ngx_max_module + 1
...
ngx_cycle_modules(ngx_cycle_t *cycle)
{
    ...
    // 拷贝make生成的静态模块数组到本cycle, 之后ngx_modules数组不再使用
    ngx_memcpy(cycle->modules, ngx_modules,
               ngx_modules_n * sizeof(ngx_module_t *));

    // 拷贝模块序号计数器到本cycle, 同样之后ngx_modules_n不再使用   
...
/**
 * 初始化cycle->modules索引ctx_index。
 * 
 * 这个字段的值将决定HTTP模块应用于请求时的顺序
 * 
 * 统计每个模块类型下面总共多少个模块
 * 
 * 返回值为指定模块类型的数量
...
ngx_count_modules(ngx_cycle_t *cycle, ngx_uint_t type)
{
    ...
    for (i = 0; cycle->modules[i]; i++) {
        ...
        //跳过非指定模块类型的。type=NGX_HTTP_MODULE
        if (module->type != type) {
            continue;
        }
        ...
        if (module->ctx_index != NGX_MODULE_UNSET_INDEX) {
            ...
            if (module->ctx_index > max) {
                max = module->ctx_index;        //记录最大的ctx_index
            }
