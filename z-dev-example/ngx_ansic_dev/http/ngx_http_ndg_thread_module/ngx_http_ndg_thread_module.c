...
{
    {
        ngx_string("ndg_thread"),   //ndg_thread poolname;
        NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
        ngx_http_ndg_thread,    //install content_handler
...
static ngx_int_t ngx_http_ndg_thread_handler(ngx_http_request_t *r)
{
    ...
    //任务执行结束后向event_fd发起通知，event.handler作为通知事件的回调
    //ngx_http_ndg_thread_event_handler为content_handler恢复后需要继续执行的逻辑
...
/**
 * 任务执行结束的回调，恢复继续执行content_handler
 */
static void ngx_http_ndg_thread_event_handler(ngx_event_t *ev)
{
    ngx_thread_task_t               *task;
