...
static ngx_command_t  ngx_http_mytest_commands[] =
{

    {
        ...
        ngx_http_mytest,        //遇到mytest配置指令时的解析方法，里边安装了content_handler  ngx_http_mytest_handler
...
//myctx->stock数组中
    ...
    //这一步很重要，设置接下来父请求的回调方法。激活父请求的方法
    //不同阶段父请求的回调函数不同，需要设置让父请求继续从之前中断的位置继续
...
mytest_post_handler(ngx_http_request_t * r)
{
    ...
    //当前请求是父请求，直接取其上下文。 子请求解析的结果，已设置到父请求自定义的上下文里了
...
ngx_http_mytest_handler(ngx_http_request_t * r)
{
    ...
    //设置子请求结束时的回调方法为mytest_subrequest_post_handler
    //子请求执行结束后在ngx_http_finalize_request() 中调用  r->post_subrequest->handler(r, r->post_subrequest->data, rc);
...
//配置的子请求location中的URI是一致的
    ...
    //sr就是子请求， 由ngx_http_subrequest创建
